...
You can avoid tripping these circuit breakers by configuring a limit,
after which the logs tables will be truncated.
...
and :ref:`Operations Log Size <stats.operations_log_size>`.

Another option is to configure an expiration time for the records. In this
case, the records in the jobs tables are periodically cleared if they are older
